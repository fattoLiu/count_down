package com.fatto.library.utils.luban;
...
public class Luban implements Handler.Callback {
    ...
    private File getImageCacheFile(Context context, String suffix) {
        ...
        // 修改 Luban 原先保存压缩后的照片的文件名
        String cacheBuilder = mTargetDir + "/" +
                ((TextUtils.isEmpty(suffix) || ".jpg".equals(suffix)) ? System.currentTimeMillis() +
                        (int) (Math.random() * 1000) + ".jpg" : (mRename ?
                        "SMALL_" + suffix : suffix));
    ...
    private void launch(final Context context) {
        ...
        while (iterator.hasNext()) {
            ...
            if (Checker.isImage(path.getPath())) {
                    ...
                    public void run() {
                        try {
                            ...
                            File result = Checker.isNeedCompress(mLeastCompressSize, path.getPath
                                    ()) ?
                                    new Engine(path, getImageCacheFile(context, Checker
                                            .checkSuffix(path.getPath()))).compress() :
                ...
                mCompressListener.onError(new IllegalArgumentException("can not read the path : "
                        + path));
            }
            iterator.remove();
        }
    ...
    private File get(InputStreamProvider path, Context context) throws IOException {
        return new Engine(path, getImageCacheFile(context, Checker.checkSuffix(path.getPath())))
                .compress();
    }
    ...
    public static class Builder {
        ...
        public Builder setRename(boolean rename) {
            this.mRename = rename;
            return this;
        }
